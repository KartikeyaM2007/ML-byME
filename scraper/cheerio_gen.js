...
for (const p of pages) {
    ...
    // Load with Cheerio using XML mode to ensure strict closing tags
    const $ = cheerio.load(mainHtml, { xmlMode: true, decodeEntities: false });
    
    // Some SVGs use camelCase properties but Cheerio lowercases them if not in xmlMode, 
    // but in xmlMode it preserves them! Wait, original DOM HTML lowercases them unfortunately (from browser DOM).
    ...
    console.log(`${p.name}.tsx generated using Cheerio XML mode!`);
}
